...
public interface DailySurveyRepository extends JpaRepository<DailySurvey, Long> {
    ...
    // 연령대와 카테고리로 문진 찾기
    Page<DailySurvey> findByAgeGroupAndCategoryAndDeletedFalse(AgeGroup ageGroup, SurveyCategory category, Pageable pageable);

    // 연령대별 전체 문진 (소프트삭제 제외)
    List<DailySurvey> findByAgeGroupAndDeletedFalse(AgeGroup ageGroup);
    Page<DailySurvey> findByAgeGroupAndDeletedFalse(AgeGroup ageGroup, Pageable pageable);

    // 전체(삭제되지 않은) 문진 전체 조회
    Page<DailySurvey> findAllByDeletedFalse(Pageable pageable);

    // 카테고리 필터 조회
    Page<DailySurvey> findAllByCategoryAndDeletedFalse(SurveyCategory category, Pageable pageable);
