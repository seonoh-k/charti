...
public interface GroupAnswerRepository extends JpaRepository<GroupAnswer, Long> {
    ...
    @Query(value = "SELECT FUNCTION('DATE', ra.createdAt), ra.child.id " +
            ...
            countQuery = "SELECT COUNT(DISTINCT FUNCTION('DATE', ra.createdAt)) " +
                    "FROM GroupAnswer ra " +
                    "WHERE ra.child = :child AND ra.deleted = false")
    Page<Object[]> findDistinctGroupDatesByChild(@Param("child") Child child, Pageable pageable);
     ...
     */
    List<GroupAnswer> findByChildAndCreatedAtBetween(Child child, LocalDateTime startDateTime, LocalDateTime endDateTime);
     ...
     * @return RecordAnswer 엔티티 목록
     */
    List<GroupAnswer> findByChild_IdAndCreatedAtBetween(Long childId, LocalDateTime startDateTime, LocalDateTime endDateTime);
