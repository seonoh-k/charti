...
import com.example.demo.survey.dto.DailyAnswerResponse;
import com.example.demo.survey.dto.DailyDateSummaryDto;
...
public class DailyAnswerAdminController {
    ...
    public String viewAnswerHistory(@RequestParam(value = "memberId", required = false) Long memberId,
        ...
        if (memberId != null) {
            try {
                ...
                List<ChildHistorySummaryDto> childrenSummaries = dailyAnswerService.getChildrenWithHistorySummary(memberId);
                ...
                return "admin/surveys/dailyHistory";
            } catch (Exception e) {
    ...
    @ResponseBody
    public ResponseEntity<ApiResponse<PagingResponse<DailyDateSummaryDto>>> getChildDailyDates(
            @PathVariable("childId") Long childId,
            PagingRequest pageRequest) {

        Pageable pageable = pageRequest.toZeroBasedPageable();
        Page<DailyDateSummaryDto> dailyDatesPage = dailyAnswerService.getDailyDatesPagedByChild(childId, pageable);
        PagingResultDTO<DailyDateSummaryDto, DailyDateSummaryDto> pagingResultDTO = new PagingResultDTO<>(dailyDatesPage);
        PagingResponse<DailyDateSummaryDto> finalResponse = PagingResponse.from(pagingResultDTO);
    ...
    @ResponseBody
    public ResponseEntity<ApiResponse<List<DailyAnswerResponse>>> getAnswersForDate(@PathVariable("childId") Long childId,
                                                                                    @PathVariable("dateStr") String dateStr) {
        try {
            LocalDate date = LocalDate.parse(dateStr);
            List<DailyAnswerResponse> answers = dailyAnswerService.getQuestionsAndAnswersForDate(childId, date);
    ...
    @ResponseBody
    public ResponseEntity<ApiResponse<Void>> updateDailyAnswers(@PathVariable("childId") Long childId,
                                                                 @PathVariable("dateStr") String dateStr,
                                                                 @RequestBody List<UpdateAnswerRequestDto> updatedAnswers) {
        try {
            LocalDate dailyDate = LocalDate.parse(dateStr);
            dailyAnswerService.updateDailyAnswers(childId, dailyDate, updatedAnswers);
    ...
    @ResponseBody
    public ResponseEntity<ApiResponse<Void>> deleteDailyAnswers(@PathVariable("childId") Long childId,
                                                                 @PathVariable("dateStr") String dateStr) {
        try {
            LocalDate dailyDate = LocalDate.parse(dateStr);
            dailyAnswerService.deleteDailyAnswers(childId, dailyDate);
