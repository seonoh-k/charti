...
public class SpecialSurveyAdminController {
    ...
    public String list(
        ...
        List<SurveyCategory> categories = specialSurveyService.getDistinctCategories();

        Page<SpecialSurvey> surveys;
        if (ageGroup == AgeGroup.ALL && category == SurveyCategory.ALL) {
            // 전체
            surveys = specialSurveyService.findAllSurveys(pageable);
        } else if (ageGroup != AgeGroup.ALL && category == SurveyCategory.ALL) {
            // 연령대만 필터
            surveys = specialSurveyService.getSurveysByAgeGroup(ageGroup, pageable);
        } else if (ageGroup == AgeGroup.ALL && category != SurveyCategory.ALL) {
            // 카테고리만 필터
            surveys = specialSurveyService.getSurveysByCategory(category, pageable);
        } else {
            // 둘 다 필터
            surveys = specialSurveyService.getSurveysByAgeAndCategory(ageGroup, category, pageable);
        }
        ...
        return "admin/surveys/specialList";
    }

    // 2. 설문 작성 폼
    @GetMapping("/new")
    public String createForm(Model model) {
        model.addAttribute("survey", new SpecialSurvey());
        model.addAttribute("ageGroups", ageGroups);
    ...
    // 2. 설문 저장 처리
    @PostMapping
    public String create(@ModelAttribute SpecialSurvey survey) {
        specialSurveyService.save(survey);
        return "redirect:/admin/surveys/special";
    }

    // 3. 설문 수정 폼
    @GetMapping("/{id}/edit")
    public String editForm(@PathVariable Long id, Model model) {
        SpecialSurvey survey = specialSurveyService.findById(id);
