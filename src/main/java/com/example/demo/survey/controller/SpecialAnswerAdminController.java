...
import com.example.demo.survey.dto.SpecialAnswerResponse;
import com.example.demo.survey.dto.SpecialDateSummaryDto;
...
@Slf4j
@RequestMapping("/admin/surveys/special")
public class SpecialAnswerAdminController {
    ...
    public String viewAnswerHistory(@RequestParam(value = "memberId", required = false) Long memberId,
        ...
        if (memberId != null) {
            try {
                ...
                List<ChildHistorySummaryDto> childrenSummaries = specialAnswerService.getChildrenWithHistorySummary(memberId);
                ...
                return "admin/surveys/specialHistory";
            } catch (Exception e) {
    ...
    @ResponseBody
    public ResponseEntity<ApiResponse<PagingResponse<SpecialDateSummaryDto>>> getChildSpecialDates(
            @PathVariable("childId") Long childId,
            PagingRequest pageRequest) {

        Pageable pageable = pageRequest.toZeroBasedPageable();
        Page<SpecialDateSummaryDto> specialDatesPage = specialAnswerService.getSpecialDatesPagedByChild(childId, pageable);
        PagingResultDTO<SpecialDateSummaryDto, SpecialDateSummaryDto> pagingResultDTO = new PagingResultDTO<>(specialDatesPage);
        PagingResponse<SpecialDateSummaryDto> finalResponse = PagingResponse.from(pagingResultDTO);
    ...
    @ResponseBody
    public ResponseEntity<ApiResponse<List<SpecialAnswerResponse>>> getAnswersForDate(@PathVariable("childId") Long childId,
                                                                                      @PathVariable("dateStr") String dateStr) {
        try {
            LocalDate date = LocalDate.parse(dateStr);
            List<SpecialAnswerResponse> answers = specialAnswerService.getQuestionsAndAnswersForDate(childId, date);
    ...
    @ResponseBody
    public ResponseEntity<ApiResponse<Void>> updateSpecialAnswers(@PathVariable("childId") Long childId,
                                                                 @PathVariable("dateStr") String dateStr,
                                                                 @RequestBody List<UpdateAnswerRequestDto> updatedAnswers) {
        try {
            LocalDate specialDate = LocalDate.parse(dateStr);
            specialAnswerService.updateSpecialAnswers(childId, specialDate, updatedAnswers);
    ...
    @ResponseBody
    public ResponseEntity<ApiResponse<Void>> deleteSpecialAnswers(@PathVariable("childId") Long childId,
                                                                 @PathVariable("dateStr") String dateStr) {
        try {
            LocalDate specialDate = LocalDate.parse(dateStr);
            specialAnswerService.deleteSpecialAnswers(childId, specialDate);
